...
} from '../../programs/program-a/program-a-application-sections.contract';
...
export class ApplicationSectionsService {
    ...
    return this.applicationsRepository.transaction(async (db) => {
      ...
      if (application.call.type !== ProgramType.PROGRAM_A) {
        throw new ConflictException(
          APPLICATIONS_MESSAGES.SECTIONS_PROGRAM_A_ONLY,
        );
      }
  ...
  async getSectionHistory(
    ...
    if (!section) {
      throw new NotFoundException(APPLICATIONS_MESSAGES.SECTION_NOT_FOUND);
    }
      ...
      if (dto.version !== null) {
        ...
        if (!historyEntry) {
          throw new BadRequestException(
            APPLICATIONS_MESSAGES.VERSION_NOT_FOUND_IN_HISTORY,
          );
        }
      }
  ...
  private assertProfileSectionValue(
    ...
    if (
      ...
      throw new BadRequestException(
        APPLICATIONS_MESSAGES.PROFILE_SECTION_PAYLOAD_INVALID,
      );
    }
  }
}
